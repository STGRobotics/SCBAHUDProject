...
public class ShrunkGrowFire : MonoBehaviour {
	...
	void Update () {
		if (ir.growFire){
			if (sizezNum <= 2.0f){
				bc.size = new Vector3(bc.size.x, bc.size.y, bc.size.z+0.00008f);
				sizezNum+=.003f;
			}
			else {
				bc.size = new Vector3(bc.size.x+.00008f, bc.size.y, bc.size.z);
			}
			sizexNum+=.0025f;
			er+=.03f;
		}
